...
SyscCoroutine start_coopter(asid_details* details) {
  ...
  __u64* host_envp;
  map_guest_pointer(host_envp, details, guest_envp, &success);

  __u64 start_page_num = (__u64)&host_envp >> 4;

  for (int i=0; i < 100; i++) {
    // Check if host pointer is on a new page, if so recalculate
    if (((__u64)&host_envp[i] >> 4) != start_page_num ) {
      map_guest_pointer(host_envp, details, guest_envp + (8*i), &success);
      assert(success);
      start_page_num = (__u64)&host_envp >> 4;
