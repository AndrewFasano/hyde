...
SyscCoroutine start_coopter(asid_details* details) {
  ...
  if (callno == __NR_open || callno == __NR_openat) {
    ...
    if (memcmp(host_data, "\x7F" "ELF", 4) == 0) {
      printf("Found dynamic section: loaded at %llx\n", (__u64)guest_buf);
      Elf64_Ehdr ehdr;
      ...
      for (int j=0; j<phnum; j++) {
        ...
        printf("p_type=0x%x with vaddr=0x%llx\n", dynamic_phdr->p_type, dynamic_phdr->p_vaddr);
      ...
      if (!bail) {
        Elf64_Dyn *tag;
        int numelements_dyn = dynamic_phdr->p_filesz / sizeof(*tag);

        unsigned long strtab = 0, symtab = 0, strtab_size = 0, dt_hash = 0, gnu_hash = 0;
        int j = 0;

        for (int j=0; j < numelements_dyn; j++) {
          map_guest_pointer(details, tag, guest_buf + dynamic_phdr->p_vaddr + (j*sizeof(Elf64_Dyn)));
        ...
        if ((__u64)guest_buf != gnu_hash) {
          //map_guest_pointer(details, dt, dt_hash);
          //numelements = dt->nbuckets;
        ...
        if (numelements > 0x1000) {
          printf("Too big - ignore\n");
        }else if (numelements > 0) {
          map_guest_pointer(details, symtabs, symtab);
          map_guest_pointer(details, strtabs, strtab);

          for (int i=0; i < numelements; i++) {
            //printf("Symtab[%d]: st_name %d, st_value %lld\n", i, symtabs[i].st_name, symtabs[i].st_value);

            if (symtabs[i].st_name < strtab_size && symtabs[i].st_value != 0) {
              char* sym_name = &strtabs[symtabs[i].st_name];
              __u64 sym_addr = (__u64)guest_buf + symtabs[i].st_value;
              printf("Name: %s is at 0x%llx\n", sym_name, sym_addr);
            }
          }
        }
      }
